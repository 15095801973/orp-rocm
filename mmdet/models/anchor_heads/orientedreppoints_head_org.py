...
class OrientedRepPointsHead(nn.Module):
    ...
    def forward_single(self, x):
        ...
        # TODO +0 也有精度的损失 DELed
        # pts_out_init = pts_out_init + points_init

        # refine and classify reppoints
        # pts_out_init_grad_mul = (1 - self.gradient_mul) * pts_out_init.detach() + self.gradient_mul * pts_out_init

        # TODO 显示指定为torch。tensor类型，而不是python浮点数，可能会有损失
        pts_out_init_grad_mul = (1 - self.gradient_mul).type_as(pts_out_init) * pts_out_init.detach() \
            + self.gradient_mul.type_as(pts_out_init) * pts_out_init
    ...
    # def train(self, mode=True):
        # super(OrientedRepPointsHead, self).train(mode)
        # self._freeze_stages()
