...
class OrientedRepPointsHead(nn.Module):
    def __init__(self,
                 ...
                 loss_border_dist_init = dict(type='BorderDistLoss', loss_weight=0.02),
                 loss_border_dist_refine = dict(type='BorderDistLoss', loss_weight=0.08)
                 ):
    ...
    def _init_layers(self):
        ...
        if self.my_pts_mode == "demo" or self.my_pts_mode == "pts_down" or self.my_pts_mode == "pts_up" or self.my_pts_mode == "int":
            self.reppoints_cls_conv = DeformConv(self.feat_channels,
                                                 self.point_feat_channels,
                                                 self.dcn_kernel, 1, self.dcn_pad)
        # todo
    ...
    def forward_single(self, x):
        ...
        if self.my_pts_mode == "core_v3":
            ...
            dcn_cls_feat = self.reppoints_cls_conv(cls_feat, dcn_offset) + self.pseudo_dcn_cls(cls_feat, core_offset) #+ cls_feat

            cls_out = self.reppoints_cls_out(self.relu(dcn_cls_feat))
            pts_refine_temp = self.reppoints_pts_refine_conv(pts_feat, dcn_offset) + self.pseudo_dcn_pts(pts_feat, core_offset)
            pts_out_refine = self.reppoints_pts_refine_out(self.relu(pts_refine_temp))
            pts_out_refine = pts_out_refine + pts_out_init.detach()
        if self.my_pts_mode == "ide2": #不变定位, 只动分类
        ...
        if self.my_pts_mode == "ide3": #不变定位, 只动分类
    ...
    def points_quality_assessment(self, points_features, cls_score, pts_pred_init, pts_pred_refine, label, rbbox_gt, label_weight, rbox_weight, pos_inds):
        ...
        qua = qua_cls + 0.2 * (qua_loc_init + 0.3 * qua_ori_init + 0.1* loss_border_dist_init) + 0.8 * (
                qua_loc_refine + 0.3 * qua_ori_refine + 0.1 *loss_border_dist_refine) + 0.1 * pts_feats_dissimilarity
